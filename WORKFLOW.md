...
- `powershell -File etc/set-status.ps1 -Chapter {NNN} -Stage draft -Arc {N}`
...
- `set-status.ps1 -Chapter {NNN} -Stage qa -Verdict <verdict>`
- audit ตอนนั้น: `audit-workspace.ps1 -Start {NNN} -End {NNN} -CheckText`
...
2. ตัดสินศัพท์ค้างใน `human-review-needed.md` → **OKF freeze** (ลง `okf/arc-freeze-log.md`)
3. audit ทั้ง arc: `audit-workspace.ps1 -Start {START} -End {END} -CheckText`
4. gate `check-arc-phase.ps1 -Arc {N} -Phase B` ต้องผ่านก่อนเข้า Phase B
...
- `set-status.ps1 -Chapter {NNN} -Stage edited`
- จบเมื่อทุกตอน `Edited` → gate `check-arc-phase.ps1 -Arc {N} -Phase C`
...
- `set-status.ps1 -Chapter {NNN} -Stage final`
- จบเมื่อทุกตอน `Final` → `check-arc-phase.ps1 -Arc {N} -Phase ship` → **ส่งมอบเล่ม**
