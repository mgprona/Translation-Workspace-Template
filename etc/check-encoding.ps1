...
# pattern จับ 3 ลายเซ็น (กัน false-positive กับคำไทยจริงที่ขึ้นต้น "เธ"/"เน" เช่น "เธอ" "เน้น"):
#   1) "เธ"/"เน" ตามด้วยอักขระนอกบล็อกไทย (control/Latin-1/€/smart-quote) อย่างน้อย 2 จุด — mojibake แทรกอักขระพวกนี้เสมอ
#   2) "เน€เธ" — ลายเซ็นตรงของ E0 B9 80 ที่เพี้ยน
#   3) run ของ "เธ"/"เน" + อักษรไทย ยาว >=5 ชุดติดกัน — mojibake ไทยล้วนทั้งวลี (คำไทยจริงไม่ต่อยาวขนาดนี้โดยไม่มีเว้นวรรค)
$mojibakePattern = '((เธ|เน)[^฀-๿ -~]).*?((เธ|เน)[^฀-๿ -~])|เน€เธ|((เธ|เน)[ก-๛]){5,}'
